...
def parse_args():
    ...
    parser.add_argument('--path', default='result9', help='input image path') # result9
    parser.add_argument('--ckdir', default='checkpoint/cat_back', help='input checkpoint path')
    ...
    parser.add_argument('--optuna_num', default=5000, help='choose optuna trial number')
    parser.add_argument('--dog_cat', default='cat', help='dog or cat')
    parser.add_argument('--class_num', default=4, help='len(class)')
    parser.add_argument('--ensemble_indicator', default='O',
...
class Generate_ensemble:
    ...
    def model_voting_proba(self, trial):
        ...
        if isinstance(trial, int):
            model_dict_proba = [1] * self.length
        ...
        tmp_test_result_list, _, _, _ = output_result(self._results_list, self.model_dict, model_dict_proba)
        ...
        best_acc, _ = calculate_accuracy(model_dict_proba, pred_result, labels, best_acc=0,
                                         best_model_dict_proba=None)
        return best_acc

    def final_result(self, model_dict_proba, dog_cat):
        # this is for final ensemble
        y_pred, y_score, eval_one_model, test = load_ckp_result(self.model_dict, self.args, model_dict_proba)
...
def main():
    ...
    print('best_model_proba : ', list(model_dict_proba.values()), '->', study.best_value)
    # show best ensemble result
    generate.final_result(list(model_dict_proba.values()), args.dog_cat)
    print('end')
