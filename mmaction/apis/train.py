...
def train_model(model,
    ...
    if cfg.omnisource:
        ...
        data_loaders = [ build_dataloader(ds, **setting) for ds, setting in zip(dataset, dataloader_settings)]
    ...
    if cfg.get('precise_bn', False):
        ...
        dataloader_setting = dict(
            ...
            dist=distributed,
            seed=cfg.seed)
        data_loader_precise_bn = build_dataloader(precise_bn_dataset,**dataloader_setting)
        precise_bn_hook = PreciseBNHook(data_loader_precise_bn,**cfg.get('precise_bn'))
