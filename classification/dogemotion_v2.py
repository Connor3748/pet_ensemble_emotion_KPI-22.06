...
class DogNet(Dataset):
    def __init__(self, stage, configs, tta=False, tta_size=48):
        ...
        glob(osp.join('..', 'dataset', 'Validation', '*', '**'))
        ...
        if stage == "train":
            ...
            #     image_path = self.img_path[i]
            #     label, _ = self.labelfind(image_path)
            #     # if label > 1:
            #     #     img = cv2.imread(image_path)
            #     #     cv2.imwrite(f'testt3/{i}.jpg', img)
    ...
    def __getitem__(self, idx):
        ...
        dataname, label = image_path.split('/')[-2], labelfind(image_path)
