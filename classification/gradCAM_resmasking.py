...
if __name__ == "__main__":
    ...
    from models.resmasking import resmasking_dropout1, resmasking_dropout2,resmasking_dropout3, resmasking_dropout4, resmasking50_dropout1, resmasking
    from models import cbam_resnet50

    model = resmasking(3, 6)
    aa = ['resmasking_test_resmasking_0305_2022Mar07_00.35']
    for iii in aa:
        os.makedirs(f'./result2/{iii}', exist_ok=True)
        # state = torch.load('./saved/checkpoints/resmasking_dropout1_rot30_2019Nov17_14.33')
        state = torch.load(f"../checkpoint/{iii}", map_location='cuda:0')
        ...
        path = '../../../dataset/dogface/testlll'
        test_img = glob.glob(os.path.join(path, "val/*.jpg"))
        # for image_path in natsorted(glob.glob('/home/z/research/archive_for_tee/face_CK+/only_face/*.png')):
        for image_path in natsorted(test_img):
            ...
            labelpath = os.path.join('../../../dataset/dog/Validation', image_path.split('/')[-1].split('__')[0] + '.json')
            f = codecs.open(labelpath, 'r')
            data = json.load(f)
            target_index = diction.index(data['metadata']['inspect']['emotion'])
            # target_index = None
            ...
            cv2.imwrite("./result2/{}/{}".format(iii,image_name), np.uint8(255 * cam))