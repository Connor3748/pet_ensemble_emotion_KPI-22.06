...
from classification.radam import RAdam
...
class FER2013Trainer(Trainer):
    ...
    def __init__(self, model, train_set, val_set, test_set, configs):
        ...
        # self._model.fc = nn.Linear(256, configs["num_classes"])
        self._model = self._model.to(self._device)

        if self._distributed == 1:
            torch.distributed.init_process_group(backend="nccl")
            self._model = nn.parallel.DistributedDataParallel(self._model)
            self._train_loader = DataLoader(self._train_set, batch_size=self._batch_size, num_workers=self._num_workers,
                                            pin_memory=True, shuffle=True, worker_init_fn=lambda x: np.random.seed(x))
            self._val_loader = DataLoader(self._val_set, batch_size=self._batch_size, num_workers=self._num_workers,
                                          pin_memory=True, shuffle=False, worker_init_fn=lambda x: np.random.seed(x))
            self._test_loader = DataLoader(self._test_set, batch_size=1, num_workers=self._num_workers, pin_memory=True,
                                           shuffle=False, worker_init_fn=lambda x: np.random.seed(x))
        else:
            self._train_loader = DataLoader(self._train_set, batch_size=self._batch_size, num_workers=self._num_workers,
        ...
        if self._configs["weighted_loss"] == 0:
            self._criterion = nn.CrossEntropyLoss(label_smoothing=0.2).to(self._device)
        else:
            self._criterion = nn.CrossEntropyLoss(class_weights, label_smoothing=0.2).to(self._device)

        self._optimizer = RAdam(params=self._model.parameters(), lr=self._lr, weight_decay=self._weight_decay, )
        self._scheduler = ReduceLROnPlateau(self._optimizer, patience=self._configs["plateau_patience"], min_lr=1e-6,
                                            verbose=True, )
        ...
        log_dir = os.path.join(self._configs["cwd"], self._configs["log_dir"],
                               "{}_{}_{}.pt".format(self._configs["arch"], self._configs["model_name"],
    ...
    def _train(self):
        ...
        for i, (images, targets) in tqdm(enumerate(self._train_loader), total=len(self._train_loader), leave=False):
    ...
    def train(self):
        ...
        try:
            ...
            # state = torch.load('../checkpoint/resnet34_test_2022Jan10_23.35', map_location=torch.device('cuda:1'))
            state = torch.load(self._checkpoint_path, map_location=torch.device(self._device))
